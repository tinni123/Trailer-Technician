...
class Update(object):
    ...
    def _check_new_version(self):
        if not config['updates']['auto_update'] or config['updates']['auto_update'] == '':
            log.info('Version checking is disabled, not checking for the newest version')
            return False

        log.info('Checking if {} needs an update'.format(self._git))
    ...
    def _get_current_version(self, git):
        output, err, exit_status = self._run_git_cmd(git, 'rev-parse HEAD')  # @UnusedVariable
...
class CheckVersion(object):
    ...
    def check_for_new_version(self, force=False):
        ...
        """
        if not config['updates']['auto_update'] and not force:
            # logger.log(u'Version checking is disabled, not checking for the newest version')
            log.info('Version checking is disabled, not checking for the newest version')
            return False

        # logger.log(u'Checking if {install} needs an update'.format(install=self.install_type))
        log.info('Checking if {} needs an update'.format(self.install_type))
        if not self.updater.need_update():
...
class GitUpdateManager(UpdateManager):
    ...
    def _find_working_git(self):
        ...
        if not config['updates']['git_path'] == '':
            main_git = '"{git}"'.format(git=config['updates']['git_path'])
        else:
            main_git = 'git'

        # logger.log(u'Checking if we can use git commands: {git} {cmd}'.format
                #    (git=main_git, cmd=test_cmd), logger.DEBUG)
        log.info('Checking if we can use git commands: {} {}'.format(main_git, test_cmd))
        output, err, exit_status = self._run_git(main_git, test_cmd)

        if exit_status == 0:
            # logger.log(u'Using: {git}'.format(git=main_git), logger.DEBUG)
            log.info('Using: {}'.format(main_git))
            return main_git
        else:
        ...
        if alternative_git:
            ...
            for cur_git in alternative_git:
                # logger.log(u'Checking if we can use git commands: {git} {cmd}'.format
                #            (git=cur_git, cmd=test_cmd), logger.DEBUG)
                log.debug('Checking if we can use git commands: {} {}'.format(cur_git, test_cmd))
